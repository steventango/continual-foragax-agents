...
class DirectionalTMaze:
    ...
    def get_sign_state(self):
        if (self.goal_is_up and self.direction == "U") or (
            (not self.goal_is_up) and self.direction == "D"
        ):
    ...
    def is_successful(self):
        if self.is_at_goal():
            if (self.goal_is_up and self.y == 1) or (
                (not self.goal_is_up) and self.y == -1
            ):
                return True
        return False
        ...
        match action:
            case 0:
                if x == self.corridor_length:
                    return (
                        self.right_goal_reward
                        if self.goal_is_up
                        else self.wrong_goal_reward
                    )
                else:
                    return self.other_state_reward + self.gamma * self.get_state_value(
                        x
                    )
            ...
            case 0:
                match self.direction:
                    ...
                    case "D":
                        self.direction = "R"
                return (
                    jnp.float32(self.get_state()),
                    jnp.float32(self.get_reward()),
                    False,
                    False,
                    self.get_info(),
                )
    ...
    def get_info(self):
        return {"gamma": self.gamma, "success": self.is_successful()}
