# ActorCriticPConv: PPO with 1×1 pointwise conv before 3×3 conv
...
class ActorCriticPConv(nn.Module):
    action_dim: int
    d_hidden: int = 192
    hidden_size: int = 64
    activation: str = "tanh"
    ...
    def __call__(self, hidden, obs):
        ...
        # Actor path: PConv (1×1) → activation → Conv (3×3) → activation → flatten → concat → dense
        actor_embedding = nn.Conv(16, (1, 1), 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="actor_pconv1")(obs)
        if self.use_layernorm:
            actor_embedding = nn.LayerNorm(epsilon=1e-05, name="actor_pconv_ln1")(actor_embedding)
        actor_embedding = activation(actor_embedding)
        actor_embedding = nn.Conv(16, (3, 3), 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="actor_conv1")(actor_embedding)
        if self.use_layernorm:
        ...
        # Critic path: PConv (1×1) → activation → Conv (3×3) → activation → flatten → concat → dense
        critic_embedding = nn.Conv(16, (1, 1), 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="critic_pconv1")(obs)
        if self.use_layernorm:
            critic_embedding = nn.LayerNorm(epsilon=1e-05, name="critic_pconv_ln1")(critic_embedding)
        critic_embedding = activation(critic_embedding)
        critic_embedding = nn.Conv(16, (3, 3), 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="critic_conv1")(critic_embedding)
        if self.use_layernorm:
