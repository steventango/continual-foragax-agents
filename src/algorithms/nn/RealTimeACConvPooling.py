...
class RealTimeActorCriticConvPooling(nn.Module):
    ...
    def __call__(self, hidden, obs):
        ...
        obs_hidden_size = (
            self.hidden_size - last_action_encoded.shape[-1] - last_reward.shape[-1]
        )
        ...
        actor_embedding = jnp.mean(
            actor_embedding, axis=(1, 2)
        )  # global average pooling over spatial dimensions
        actor_embedding = jnp.append(
            actor_embedding,
            jnp.append(last_action_encoded, last_reward, axis=1),
            axis=1,
        )
        ...
        critic_embedding = jnp.mean(
            critic_embedding, axis=(1, 2)
        )  # global average pooling over spatial dimensions
        critic_embedding = jnp.append(
            critic_embedding,
            jnp.append(last_action_encoded, last_reward, axis=1),
            axis=1,
        )
