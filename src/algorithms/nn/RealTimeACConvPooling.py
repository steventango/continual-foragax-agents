...
class RealTimeActorCriticConvPooling(nn.Module):
    ...
    def __call__(self, hidden, obs):
        ...
        actor_embedding = nn.Conv(obs_hidden_size, 3, 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="actor_conv")(obs)
        actor_embedding = activation(actor_embedding)
        actor_embedding = jnp.mean(actor_embedding, axis=(1, 2))  # global average pooling over spatial dimensions
        ...
        critic_embedding = nn.Conv(obs_hidden_size, 3, 1, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="critic_conv")(obs)
        critic_embedding = activation(critic_embedding)
        critic_embedding = jnp.mean(critic_embedding, axis=(1, 2))  # global average pooling over spatial dimensions
