...
# Non Linear RTUs without gradient corrections in forward pass
class NonLinearRTUs(nn.Module):
    ...
    def __call__(self,h_tminus1,x_t): 
        ...
        h_t_c1 = act_options[self.activation](jnp.multiply(g,h_tminus1_c1) - jnp.multiply(phi,h_tminus1_c2) + jnp.multiply(norm,w_c1_x_t))
        h_t_c2 = act_options[self.activation](jnp.multiply(g,h_tminus1_c2) + jnp.multiply(phi,h_tminus1_c1) + jnp.multiply(norm,w_c2_x_t))
...
class RealTimeNonLinearRTUs(nn.Module):
    ...
    def __call__(self,carry,x_t):
        ...
        model_fn = FwdRealTimeNonLinearRTUs(n_hidden=self.n_hidden,params_type=self.params_type,stable_r=self.stable_r,activation=self.activation)
