...
class RealTimeActorCriticConv(nn.Module):
    ...
    def __call__(self, hidden, obs):
        ...
        actor_embedding = nn.Dense(self.hidden_size, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="actor_dense1")(actor_embedding)
        actor_embedding = activation(actor_embedding)
        ...
        critic_embedding = nn.Dense(self.hidden_size, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0), name="critic_dense1")(critic_embedding)
        critic_embedding = activation(critic_embedding)
