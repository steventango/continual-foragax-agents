...
class NetworkBuilder:
    ...
    def getActionTraceMultiplicativeActionRecurrentFeatureFunction(self):
        def _inner(params: Any, x: jax.Array | np.ndarray, a: jax.Array, action_trace: jax.Array, reset: jax.Array | np.ndarray = None, carry: jax.Array | np.ndarray = None, is_target = False):
            return self._feat_net.apply(params['phi'], x, a, action_trace, reset=reset, carry=carry, is_target=is_target)
...
def buildFeatureNetwork(inputs: Tuple, params: Dict[str, Any], rng: Any):
    ...
    def _inner(x: jax.Array, *args, **kwargs):
        ...
        if name == "TwoLayerCrelu":
            ...
            # It uses initializer different from above
            net = ForagerAAGRUNetReLU(hidden=hidden, actions=params["actions"], learn_initial_h=params.get('learn_initial_h', True), name='ForagerAAGRUNetReLU')
            return net(x, *args, **kwargs)
        
        elif name == 'ForagerATAAGRUNetReLU':
            # It uses initializer different from above
            net = ForagerATAAGRUNetReLU(hidden=hidden, actions=params["actions"], learn_initial_h=params.get('learn_initial_h', True), name='ForagerATAAGRUNetReLU')
            return net(x, *args, **kwargs)
...
class ATAAGRU(hk.Module):
    ...
    def gru_call(self, inputs, action, action_trace, state):
        ...
        w_h = hk.get_parameter("w_h", [self.hidden_size, 3 * self.hidden_size], init=self.w_init)
        w_a = hk.get_parameter("w_a", [self.number_of_actions, 3 * self.hidden_size], init=self.w_init)
        w_at = hk.get_parameter("w_at", [self.number_of_actions, 3 * self.hidden_size], init=self.w_init)
        b = hk.get_parameter("b", [3 * self.hidden_size], init=self.b_init)
        ...
        zr_x, a_x = jnp.split(gates_x, indices_or_sections=[2 * self.hidden_size], axis=-1)
        zr_a, a_a = jnp.split(gates_a, indices_or_sections=[2 * self.hidden_size], axis=-1)
        zr_at, a_at = jnp.split(gates_at, indices_or_sections=[2 * self.hidden_size], axis=-1)
    ...
    def gru_step(self, prev_state, inputs):
        ...
        prev_state = jax.lax.select(reset_flag, carry[None, :], prev_state)
        # GRU expects inputs with a batch dimension.
        output, next_state = self.gru_call(frame_feat[None, :], action, action_trace, prev_state)
        # Remove the extra batch dimension and return both output and next_state.
        return next_state, (output[0], next_state[0])

    def process_sequence(self, features_seq, action_seq, action_trace_seq, reset_seq, carry_seq):
        final_state, (outputs_seq, state_seq) = hk.scan(self.gru_step, carry_seq[:1, :], (features_seq, action_seq, action_trace_seq, reset_seq, carry_seq))
        return outputs_seq, state_seq
    
    def __call__(self, x: jnp.ndarray, a: jnp.ndarray, action_trace: jnp.ndarray, reset: jnp.ndarray = None, carry: jnp.ndarray = None, is_target = False) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        """
        Args:
          x: Input tensor with shape [N, T, ...]
          a: Action tensor with shape [N, T]
...
class ForagerATAAGRUNetReLU(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray, a: jnp.ndarray = None, action_trace = None, reset: jnp.ndarray = None, carry: jnp.ndarray = None, is_target = False) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        """
        Args:
          x: Input tensor with shape [N, T, ...]
          reset: Optional binary flag sequence with shape [N, T] indicating when to reset the GRU state.
