            ...
            for sub in ["foragax", "foragax-sweep"]:
                ...
                for config_dir in glob.glob(pattern):
                    ...
                    if suffix == "weather":
                        ...
                        source_dir = config_dir.replace(
                            f"E{i}-{suffix}", f"E{source_i}-{source_suffix}"
                        )
                    ...
                    if base_path:
                        ...
                        if (
                            "metaParameters" in config
                            and "representation" in config["metaParameters"]
                        ):
                            config["metaParameters"]["representation"]["type"] = (
                                "Forager2CreluNet"
                            )
