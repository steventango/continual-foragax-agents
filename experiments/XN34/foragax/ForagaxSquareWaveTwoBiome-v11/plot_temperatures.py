...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(1, 2, squeeze=False, sharey=True, figsize=(15, 8))
    ...
    def plot_selection(ax, family: str, aperture: int | None, desired_variant: str | None, require_l2: bool=False, require_reward_trace: bool=False, require_sinusoidal: bool=False, color_index:int=0, plot_all_lines:bool=False, metric: str="ewm_reward"):
        ...
        if family.startswith("SINGLE:"):
            ...
            for seed in seeds:
                ...
                x, y = extract_learning_curves(seed_df, hyper_vals=hyper_vals, metric=metric)
                x = np.asarray(x)
                y = np.asarray(y)
                xs.append(x)
                ys.append(y)
            ...
            if len(ys) >= 5 and not plot_all_lines:
        ...
        for ar in sub_results:
            ...
            if family == "ESM" and not is_esm(name):
                continue
            if family == "DQN" and not (name.startswith("DQN_frozen_5M") or name == "DQN"):
                continue
            ...
            if len(ys) >= 5 and not plot_all_lines:
                ax.plot(xs[0], res.sample_stat, label=label, color=variant_color, linewidth=10.0)
                ax.fill_between(xs[0], res.ci[0], res.ci[1], color=variant_color, alpha=0.5)
            else:
                for y in ys:
                    ax.plot(xs[0], y, color=variant_color, linewidth=0.2, alpha=0.5)
            return
    ...
    plot_selection(ax, "DQN", 15, "5M", color_index=1, plot_all_lines=True, metric="temperatures_1")
    plot_selection(ax, "DQN", 15, "5M", color_index=0)
    ...
    # plot_selection(ax, "PPO", 15, "5M", color_index=5, require_reward_trace=True)
    ax.ticklabel_format(axis="x", style="sci", scilimits=(0, 0), useMathText=True)
    ax.set_xlabel("Time steps")
    ax.set_ylabel("Base reward (temperatures_1)")
    ...
    # plot_selection(ax, "SINGLE:Search-Brown-Avoid-Green", None, "", color_index=-2)
    # plot_selection(ax, "RTU", 9, "5M", color_index=0)
    plot_selection(ax, "DQN", 15, "5M", color_index=1, metric="temperatures_1")
    # plot_selection(ax, "RTU", 15, "5M", color_index=2)
    # plot_selection(ax, "PPO", 9, "5M", color_index=3, require_reward_trace=True)
    # plot_selection(ax, "PPO", 5, "5M", color_index=4, require_reward_trace=True)
    # plot_selection(ax, "PPO", 15, "5M", color_index=5, require_reward_trace=True)
    ...
    # # plot_selection(ax, "SINGLE:Search-Brown-Avoid-Green", None, "", color_index=-2)
    # # RTU/PPO at FOV 9 and 15 (base, non-frozen)
    # plot_selection(ax, "PPO", 5, "", color_index=0, require_reward_trace=True)
    ...
    # plot_selection(ax, "PPO", 5, "", color_index=3, require_l2=True)
    # plot_selection(ax, "PPO", 9, "", color_index=4, require_l2=True)
    # plot_selection(ax, "PPO", 15, "", color_index=5, require_l2=True)
    # # plot_selection(ax, "PPO", 15, "", color_index=2, require_reward_trace=True)
    # ax.ticklabel_format(axis="x", style="sci", scilimits=(0, 0), useMathText=True)
    # ax.set_xlabel("Time steps")
