for fov in 5 9 15; do
    for metric in reward biome_regret; do
        for alg in dqn ppo rtu drqn_1_1 drqn_0_2; do
            ...
            python src/learning_curve.py experiments/E136-big/foragax/ForagaxBig-v5 --plot-name compare_layernorm/fig_big_${alg}_fov${fov}_${metric} --filter-alg-apertures ${base}:${fov} ${ln}:${fov} Search-5 --metric rolling_${metric}_10000
        done
    done
done