...
if __name__ == "__main__":
    ...
    def is_ppo_conv(name: str) -> bool:
        return name.startswith("ActorCriticConv")
    ...
    def pretty_label(raw_alg: str, aperture: int | None, variant: str) -> str:
        ...
        if raw_alg.startswith("Search-"):
            return raw_alg
        fam = "RTU-Conv" if is_rtu_conv(raw_alg) else ("RTU-PPO" if is_rtu(raw_alg) else ("PPO-Conv" if is_ppo_conv(raw_alg) else ("PPO" if is_ppo(raw_alg) else raw_alg)))
    ...
    def plot_selection(ax, family: str, aperture: int | None, desired_variant: str | None, position: int, start_step=0, end_step=10_000_000, require_l2: bool=False, require_reward_trace: bool=False, require_sinusoidal: bool=False, color_index:int=0):
        ...
        for ar in sub_results:
            ...
            if family == "PPO" and not is_ppo(name):
                continue
            if family == "PPO-Conv" and not is_ppo_conv(name):
                continue
    ...
    plot_selection(ax, "PPO", 9, "", end_step=1_000_000, position=2, color_index=2, require_reward_trace=True)
    plot_selection(ax, "PPO-Conv", 9, "", end_step=1_000_000, position=3, color_index=3, require_reward_trace=True)
    plot_selection(ax, "RTU", 9, "", end_step=1_000_000, position=4, color_index=4)
    plot_selection(ax, "RTU-Conv", 9, "", end_step=1_000_000, position=5, color_index=5)
