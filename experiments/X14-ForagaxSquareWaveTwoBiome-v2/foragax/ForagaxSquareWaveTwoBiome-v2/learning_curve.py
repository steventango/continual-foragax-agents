...
if __name__ == "__main__":
    ...
    def plot_selection(ax, family: str, aperture: int | None, desired_variant: str | None, require_l2: bool=False, require_reward_trace: bool=False, require_sinusoidal: bool=False, color_index:int=0, plot_all_lines:bool=False):
        ...
        if family.startswith("SINGLE:"):
            ...
            ax.plot(xs[0], res.sample_stat, label=label, color=variant_color, linewidth=5.0, alpha=0.6)
        ...
        for ar in sub_results:
            ...
            if family == "ESM" and not is_esm(name):
                continue
            if family == "DQN" and not name == "DQN":
                continue
            ...
            ax.plot(xs[0], res.sample_stat, label=label, color=variant_color, linewidth=5.0)
            if len(ys) >= 5 and not plot_all_lines:
                ax.fill_between(xs[0], res.ci[0], res.ci[1], color=variant_color, alpha=0.1)
            else:
                for y in ys:
    ...
    plot_selection(ax, "PPO", 15, "5M", color_index=1, require_reward_trace=True)
    plot_selection(ax, "PPO", 9, "5M", color_index=2, require_reward_trace=True)
    plot_selection(ax, "PPO", 5, "5M", color_index=3, require_reward_trace=True)
