...
def main(experiment_path: Path, normalize: str | None):
    ...
    for group_key, df in all_df.group_by(["alg_base", "aperture", "alg"]):
        ...
        if aperture is not None:
            col_linear = main_algs.index(alg_base)
    ...
    for i in range(len(main_algs)):
        alg_base = main_algs[i]
        alg_label = LABEL_MAP.get(alg_base, alg_base)
        row = i // ncols_mean
        col = i % ncols_mean
        # Title for mean figure
        axs_mean[row, col].set_title(f"{alg_label}")
        # Title for seeds figure (first row)
        axs_seeds[0, i].set_title(f"{alg_label}")
    ...
    save(
        ...
        save_type="pdf",
        f=fig_mean,
        width=ncols_mean if ncols_mean > 1 else 4/3,
        height_ratio=2 / 3 * nrows_mean / ncols_mean if ncols_mean > 1 else 1/2
    )
    save(
        save_path=f"{path_plots}/plots",
        ...
        save_type="pdf",
        f=fig_seeds,
        width=ncols if ncols > 1 else 4/3,
        height_ratio=(num_seeds / ncols) * (2 / 3) if ncols > 1 else num_seeds / 2
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="experiments/E54-weather/foragax/ForagaxWeather-v3",
    )
    parser.add_argument(
        "--normalize",
        type=str,
