...
if __name__ == "__main__":
    ...
    for env_aperture, sub_results in sorted(
        ...
        for alg_result in sub_results:
            ...
            # Score this agent by the area under its mean learning curve; keep only the best per FOV from env_aperture
            xgrid = xs[0]
            mean_curve = res.sample_stat
            score = float(np.trapezoid(mean_curve, xgrid))  # AUC score
            current = best_by_aperture.get(aperture)
            if (current is None) or (score > current["score"]):
                best_by_aperture[aperture] = {
                    "label": label,
                    "xs": xgrid,
                    "mean": mean_curve,
                    "ci": res.ci,
                    "color": COLORS.get(label, None),
                    "has_many_seeds": len(ys) >= 5,
                    "all_ys": ys,
                    "score": score,
                }

    # === Plot only the best agent for each aperture on a single figure ===
    ax.ticklabel_format(axis="x", style="sci", scilimits=(0, 0), useMathText=True)
    ax.set_xlabel("Time steps")
    ax.set_ylabel("Average Reward")

    # Draw oracle line (already added earlier) and the winners
    for aperture in (3, 7, 11):
        entry = best_by_aperture.get(aperture)
        if not entry:
            continue
        ax.plot(
            entry["xs"],
    ...
    ax.set_title(f"{env} — Best Agents per Aperture (3, 7, 11)")
    ...
    save(
        ...
        plot_name=f"{env}_best_apertures",
        save_type="pdf",
        f=fig,
        height_ratio=2 / 3,
    )
