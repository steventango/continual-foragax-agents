    ...
    for env, apertures in env_to_apertures.items():
        ...
        for idx, aperture in enumerate(apertures):
            ...
            for alg_result in sub_results:
                ...
                # Exclude any agent with "world" or "1M" in its name
                if "world" in alg.lower() or "1m" in alg.lower():
                    continue
                if alg in SINGLE:
                    label = alg
                else:
            ...
            # Map base_label -> color; ensure 1M and non-1M share color
            label_to_color = {}
            color_i = 0
            ...
            for alg_result in sub_results:
                ...
                # Use the shared color for 1M/non-1M variants; dash style for 1M
                base_alg = base_without_1m(alg)
                base_label = base_alg if alg in SINGLE else f"{base_alg}-{aperture}"
                ...
                if label == "Random":
                    ...
                    linestyle = "--" if "1M" in alg else "-"

                # Plot each seed individually (no CI)
