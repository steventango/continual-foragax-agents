...
if __name__ == "__main__":
    ...
    def pretty_label(raw_alg: str, aperture: int | None, variant: str, value) -> str:
        ...
        if "l2" in raw_alg:
            suffix = " L2 " + suffix
        if "v10" in raw_alg:
            suffix = " Hyper from v10 " + suffix
        else:
            suffix = " Hyper swept in v13 " + suffix
        suffix = suffix + " AUC: " + str(value)
    ...
    def plot_selection(ax, family: str, aperture: int | None, desired_variant: str | None, require_l2: bool=False, color_index:int=0, require_v10 = False):
        ...
        if family.startswith("SINGLE:"):
            ...
            label = pretty_label(single_name, None, "", np.mean(res.sample_stat))
        ...
        for ar in sub_results:
            ...
            if require_v10 and "-v10-hypers" not in name:
                continue
            if not require_v10 and "-v10-hypers" in name:
                continue
            ...
            label = pretty_label(name, aperture, vt, np.mean(res.sample_stat))
    ...
    plot_selection(ax, "RTU", 15, "", color_index = 3, require_v10=True)
    ax.ticklabel_format(axis="x", style="sci", scilimits=(0, 0), useMathText=True)
    ax.set_xlabel("Time steps")
    ax.set_ylabel("Average Reward")
    ax.set_title(f"{env} — RTU FOV 15 Old and New")
    ...
    # ax.set_xlabel("Time steps")
    # ax.set_ylabel("Average Reward")
    # ax.set_title(f"{env} — L2 & FOV 9/15")
    # ax.spines["top"].set_visible(False)
    # ax.spines["right"].set_visible(False)
    # ax.legend(ncol=1, loc="best", frameon=False, fontsize=12)
